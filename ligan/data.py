...
class AtomGridData(object):
    ...
    def forward(self, interpolate=False, spherical=False):
        ...
        ##
        feats = torch.ones((self.batch_size,33), device=self.device)
        tmp = torch.zeros((self.batch_size), device=self.device)
        ##
        examples.extract_label(0, labels)
        ##                                                                  ### continue here
        for i in range(33):
            
            examples.extract_label(i+3,tmp)
            feats[:,i] = tmp
        ##
