...
class GenerativeSolver(nn.Module):
    ...
    def init_gen_model(
        ...
        **gen_model_kws
        ...
        if state:
            ...
            self.gen_model.load_state_dict(torch.load(state), strict=False)
    ...
    def gen_forward(self, data, grid_type, fit_atoms=False):
        ...
        if posterior or has_cond: # get real examples
            input_grids, cond_grids, input_structs, cond_structs, _, _, feats =  data.forward()                                                          ### checkpoint
            
            
            # print(self.gen_model)
            # print("#############",cond_grids.shape )
            # raise SystemExit(0)
        ...
        if decode_stage2_vecs: # should only do this in test phase
            ...
            latent_vecs_gen, latent2_means, latent2_log_stds = \
                self.gen_model.forward2(
                    ...
                    features = feats if has_cond else None,
